...
export default function Input({
  ...
  return (
    <div className="w-full">
      ...
      <div className="relative">
        ...
        {isPasswordField && (
          <button
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 rounded-md p-1 text-slate-400 hover:text-slate-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
      ...
      {helpText && !error && (
        <p id={`${props.name}-help`} className="mt-1.5 text-sm text-slate-500">{helpText}</p>
      )}
    </div>
  );
}
